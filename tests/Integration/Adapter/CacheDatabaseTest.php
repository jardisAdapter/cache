...
class CacheDatabaseTest extends TestCase
{
    ...
    {
        $cache1 = new CacheDatabase($this->pdo, namespace: 'namespace1');
        $cache2 = new CacheDatabase($this->pdo, namespace: 'namespace2');
    ...
    {
        $cache1 = new CacheDatabase($this->pdo, namespace: 'app1_');
        $cache2 = new CacheDatabase($this->pdo, namespace: 'app2_');
    ...
    public function testSetWithZeroTtlDeletesItem(): void
    {
        $this->cache->set('zero_ttl', 'value');
        $this->assertTrue($this->cache->has('zero_ttl'));

        // PSR-16: TTL 0 must delete the item
        $this->cache->set('zero_ttl', 'value', 0);
        $this->assertFalse($this->cache->has('zero_ttl'));
    }

    public function testSetWithNegativeTtlDeletesItem(): void
    {
        $this->cache->set('negative_ttl', 'value');
        $this->assertTrue($this->cache->has('negative_ttl'));

        // PSR-16: negative TTL must delete the item
        $this->cache->set('negative_ttl', 'value', -10);
        $this->assertFalse($this->cache->has('negative_ttl'));
    }
